...
void showErrors(int cod)
{
	...
	{
	case 1:
		printf("A linha inicial nao esta na forma correta \n");
		break;
	case 2:
		printf("A coluna inicial nao esta na forma correta \n");
		break;
	case 3:
		printf("A linha destino nao esta na forma correta \n");
		break;
	case 4:
		printf("A coluna inicial nao esta na forma correta \n");
		break;
	case 5:
		printf("Esse espaco e vazio nao pode ser movido \n");
		break;
	case 6:
		printf("Movimento Invalido  \n");
		break;
	case 7:
		printf("Nao e permitido andar para tras \n");
		break;
	case 8:
		printf("Voce esta jogando com as pecas erradas \n");
		break;
	case 9:
		printf("Voce so pode ir em um espaco em branco \n");
		break;
	case 10:
		printf("Para realizar essa jogada precisa ter uma peca adversaria no meio \n");
		break;
	default:
...
{
	printf("\nDigite a posicao inicial da peca({número linha}{número coluna}):");
	scanf("%d", pinitLin);
	printf("\nDigite a posicao final da peca({número linha}{número coluna}):");
	scanf("%d", pdestLin);
...
int jogada(int matrizTabuleiro[9][9], int vez, int *ppontos, int *pcontnc, int *vezpoint)
{
	...
	else
	{
		...
		strcpy(jogadaArr, readDriver());
		initLin = atoi(jogadaArr[1]);
		initCol = atoi(jogadaArr[2]);
		destLin = atoi(jogadaArr[3]);
		destCol = atoi(jogadaArr[4]);
...
void instrucoes(char pj1[50], char pj2[50])
{
	...
	printf("|                INSTRUCOES                        |\n");
	printf("| PECAS:                                           |\n");
	printf("| AS PEDRAS VERMELHAS SAO REPRESENTADAS POR UM X   |\n");
	printf("| AS DAMAS VERMELHAS SAO REPRESENTADAS POR UM r    |\n");
	printf("| AS PEDRAS BRANCAS SAO REPRESENTADAS POR UM o     |\n");
	printf("| AS DAMAS BRANCAS SAO REPRESENTADAS POR UM q      |\n");
	printf("| COMO JOGAR:                                      |\n");
	printf("| 1.O JOGADOR 1(PECAS VERMELHAS) COMECARA A PARTIDA|\n");
	printf("| 2.PARA MOVIMENTAR AS PECAS DIGITE O NUMERO DA LI-|\n");
	printf("| NHA INICIAL SEGUIDO DO NUMERO DA COLUNA INICIAL. |\n");
	printf("| EXEMPLO: 34 (LINHA 3 COLUNA 4). LOGO APOS DIGITE |\n");
	printf("| O NUMERO DA LINHA DESTINO SEGUIDO DO NÚMERO DA   |\n");
...
int main()
{
	...
	while (joga == 0)
	{
		...
		{
		case 0:
			printf("Sua vez %s (PECAS x)\n", jogador1);
			break;
		case 1:
			printf("Sua vez %s (PECAS o)\n", jogador2);
		}
